            ...
            if ($excustomer) {
                ...
                if ($excustomer->doctor_status != 1) {
                    ...
                    if ($request->via == '4') {
                        ...
                        $res = $this->chargiartimedoctor($request->Caller_Number, $product->id, $product->price);
                        if ($res) {
                    ...
                    if ($request->via == '1') {
                        $res = $this->chargiartime($request->Caller_Number, $product->id, $product->price);
                        if ($res) {
                        ...
                        if ($excustomer->ivr_enticement == 0) {
                            ...
                            $resmpesa = $this->chargempesa($request->Caller_Number, $request->productID, $product->price);
                            return $resmpesa;
                        }
                    }
                } else {
                ...
                if ($request->via == '1') {
                    ...
                    if ($data['output_ResponseCode'] == 0) {
                        ...
                        $costomr = Customer::where('msisdn', $request->Caller_Number)->get()->first();
                        if ($costomr) {
                            $costomr->ivr_enticement = 0;
                            $costomr->save();
                        }
                        $resp = array(
                            "status" => "77",
    ...
    public function subscriptionstatus(Request $request)
    {
        ...
        if ($excustomer) {
            if ($excustomer->ivr_status == 1) {
                $subscrb = Subscription::where('customer_ID', $excustomer->id)->where('product_id', 1)->get()->first();
                if ($subscrb) {
                    $resp = array(
                        "ivr_status" => "1",
			...
			"starts_at" => Carbon::now()->toDateTimeString(),
                        "ends_at" => Carbon::now()->addDays(1)->toDateTimeString(),
		);
		    Log::info($resp);
                    return response()->json($resp);
                }
